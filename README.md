...
| **桌面浮窗** | 独立的置顶胶囊窗口，可拖动、宽度自适应，空闲时收成纯宠物。见下文 |
| **Codex 宠物** | 直接用你在 Codex 里装的宠物，自动跟随当前选中那只，动作按状态自动切换 |
...
选项
  显示计时器            当前这轮已经跑了多久
  桌面浮窗              开关独立的置顶浮窗
  浮窗外观 ▸            图标大小：小/中/大/特大    动画速度：跟随GIF/慢/正常/快
  思考词 ▸              可爱（带颜文字）/ 简洁 / 英文原版 / 关闭
  动画 ▸                内置：Claude Spark / Claude Code / 螃蟹漫步
                        Codex 宠物：自动（跟随 Codex）/ 各个已装宠物
  颜色 ▸                橙色 / 跟随系统
...
上表的圆点是**内置动画模式**下的表现。选了 Codex 宠物之后，每个状态都会播放宠物对应的动作，不再退化成圆点。
...
菜单栏太挤、或者你想让状态更显眼时可以开它：**选项 → 桌面浮窗**。

```
╭──────────────────────────────╮
│  🦀  摸鱼中… (￣▽￣)  1m 23s  │   ← 置顶、可拖动、宽度随内容伸缩
╰──────────────────────────────╯
```

浮窗是个**独立的 Rust 程序**（`float/` 目录），不是 Swift App 的一部分。它和菜单栏读同一份状态文件与配置，所以显示内容和配色始终一致；菜单栏里改底色、圆角、思考词风格、动画样式，浮窗会跟着变。
...
如果你装过 [Codex](https://developers.openai.com/codex) 的宠物，**菜单栏和浮窗可以直接用它们**，而且会自动跟随你在 Codex 里选中的那只：

```
选项 ▸ 动画 ▸ Codex 宠物 ▸ 自动（跟随 Codex）
```

宠物装在 `$CODEX_HOME/pets` 或 `~/.codex/pets`，可以用 [petdex](https://petdex.dev) 或 [codex-pets](https://github.com/YaKun9/codex-pets) 安装。**本项目不分发任何宠物素材**，只读取你本地已装的，版权归各宠物作者（原创多为 CC BY-NC-SA，同人作品限制更严）。

**动作会跟着状态自动切换**，不需要手动指定。映射依据是 Codex `hatch-pet` skill 里定义的 9 个标准动作行：

| 本项目状态 | Codex 动作行 | 说明 |
|---|---|---|
| 思考中 | `review` (8) | 审阅 |
| 调用工具 | `running` (7) | 干活 |
| 等待授权 | `waiting` (6) | skill 原文：needs approval, help, or user input |
| 出错 | `failed` (5) | |
| 已完成 | `waving` (3) | 挥手 |
| 空闲 | `idle` (0) | 待机 |
| **鼠标悬停** | `waving` (3) | 移上去打招呼 |

雪碧图规格：单帧 192×208、8 列、9 或 11 行（`spriteVersionNumber` 1/2 的区别），行尾空白格会自动跳过。这套规格官方没有公开文档，是从 `hatch-pet` skill 的 SKILL.md 里读出来的。

### 换成你自己的动画

把任意 GIF 放到这个路径就生效，不用改配置：

```
~/.claude/statusbar/anim.gif
```

- 文件存在就优先用它，**删掉自动退回内置螃蟹**
- **改了文件自动重载**，按 mtime 检测，不用重启
- **播放速度取 GIF 自己的帧延迟**，不写死
- **宽高比自适应**，非方形也不会被拉变形
